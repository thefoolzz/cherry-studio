...
const NAVIGATE_TOOL: Tool = {
  name: 'navigate',
  description:
    'Create a clickable entry for a route returned by product_info. Use this in the same turn whenever answering where to find, open, configure, or use a chenwei page or feature; written UI steps are not a substitute.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
...
const DIAGNOSE_TOOL: Tool = {
  name: 'diagnose',
  description:
    'Read chenwei runtime state for troubleshooting. Use this to inspect app info, provider config, connectivity, logs, and MCP server status.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
...
const PRODUCT_INFO_TOOL: Tool = {
  name: 'product_info',
  description:
    'Read current chenwei product facts from the installed package manifest. Request only the relevant section to keep context small.',
  inputSchema: {
    type: 'object',
    properties: {
      source: {
...
The tool returns the new agent details, and chenwei presents a Go to chat action. Do not call navigate after a successful creation.`,
  inputSchema: {
    type: 'object',
    properties: {
      name: {
...
const APPLY_SETTING_TOOL: Tool = {
  name: 'apply_setting',
  description: `Apply a low-risk chenwei setting change directly. Only the whitelist below is supported; destructive operations are never exposed here.
  ...
  private async createAgent(args: Record<string, string | undefined>) {
    ...
    try {
      ...
      if (isDataApiError(error) && error.code === DataApiErrorCode.NOT_FOUND) {
        throw new McpError(ErrorCode.InvalidParams, `Model is not configured in chenwei: ${parsedModel.data}`)
      }
      throw error
    }
