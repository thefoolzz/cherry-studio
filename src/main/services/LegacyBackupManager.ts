...
class BackupManager {
  ...
  private async restoreUnlocked(backupPath: string): Promise<void> {
    ...
    try {
      ...
      if (!(await fs.pathExists(path.join(extractionDir, 'metadata.json')))) {
        throw new Error(`Unsupported v1 backup. chenwei v2 can only restore backup version ${DIRECT_BACKUP_VERSION}.`)
      }
  ...
  private async restoreDirect(extractionDir: string): Promise<void> {
    ...
    if (existingJournal.kind === 'corrupt') {
      throw new Error('A corrupt restore journal already exists. Restart chenwei before trying again.')
    }
    if (
      existingJournal.kind === 'ok' &&
      (existingJournal.journal.state === 'staged' || existingJournal.journal.state === 'promoting')
    ) {
      throw new Error('Another restore is already pending. Restart chenwei before trying again.')
    }
  ...
  private async readDirectBackupMetadata(extractionDir: string): Promise<DirectBackupMetadata> {
    ...
    if (!raw || typeof raw !== 'object' || raw.appName !== 'Cherry Studio') {
      throw new Error('This backup file is not from chenwei and cannot be restored')
    }
    if (raw.version !== DIRECT_BACKUP_VERSION) {
      throw new Error(
        `Unsupported backup version ${String(raw.version)}. chenwei v2 can only restore backup version ${DIRECT_BACKUP_VERSION}.`
      )
    }
